...
public class AST_DEC_VAR extends AST_DEC {
	...
	public TEMP IRme()
	{
		IR.getInstance().Add_IRcommand(new IRcommand_Allocate(this.id));
		
		if (this.exp != null)
		{
		  TEMP tmp = this.exp.IRme();
		  if (tmp != null){
			IR.getInstance().Add_IRcommand(new IRcommand_Store(this.id,tmp));}
			else {
			  FunctionHelpers.vars_ubi.add(this.id);
        System.out.print(this.id + " added to ubi\n");
